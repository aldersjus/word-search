...
class Horizontal(WordBase):
    ...
    def add_to_grid(self, word_class):
        if self.check_all_points(word_class.all_points):
            ...
            self.create_word_horizontal(word_class.word)

    def create_word_horizontal(self, word):
        ...
        if not_in_list:
            ...
            if word_class.x + len(word) <= 10:  # TODO replace ten with length of grid lists

                for l in range(len(word)):
                    word_class.add_positions((word_class.y, word_class.x + l))

                self.add_to_grid(word_class)

            elif word_class.x - len(word) >= 0:
                ...
                self.create_word_horizontal(word)
