...
class Vertical(WordBase):
    ...
    def add_to_grid(self, word_class):
        if self.check_all_points(word_class.all_points):
            ...
            self.create_word_vertical(word_class.word)

    def create_word_vertical(self, word):
        ...
        if not_in_list:
            ...
            if word_class.y + len(word) <= 10:  # TODO replace ten with length of grid lists

                for l in range(len(word)):
                    word_class.add_positions((word_class.y + l, word_class.x))

                self.add_to_grid(word_class)

            elif word_class.y - len(word) >= 0:
                ...
                self.create_word_vertical(word)
        else:
            self.create_word_vertical(word)
